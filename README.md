...
```bash
mhyapp$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
mhyapp$ sam local start-api
mhyapp$ curl http://localhost:3000/
```
...
```bash
mhyapp$ sam logs -n HelloWorldFunction --stack-name mhyapp --tail
```
